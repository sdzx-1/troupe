...
Troupe's core insight is exactly this: **use this global state machine as the single source of truth**. Developers only need to describe the overall state graph of the protocol once, rather than writing code for each role separately. For example, a simple ping-pong protocol can be represented as:
...
Projection is a mathematical concept: mapping a high-dimensional object onto a lower-dimensional subspace . In Troupe, the global state machine is the high-dimensional object, containing information for all roles. Each role only needs to see the parts relevant to itself—just like observing the same three-dimensional object from different angles to yield different two-dimensional projections.

Troupe executes this projection at compile time:
